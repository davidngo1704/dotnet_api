...
namespace Api.Controllers
{
    ...
    public class BlockchainController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetBalance()
        {
            ...
            var mexcResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/mexc && source .venv/bin/activate && python balance_spot.py"
            });

            var kucoinResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/kucoin && source .venv/bin/activate && python balance_spot.py"
            });

            var bingxResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/bingx && source .venv/bin/activate && python balance_spot.py"
            });

            var okxResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/okx && source .venv/bin/activate && python balance_spot.py"
            });

            var binanceResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/binance && source .venv/bin/activate && python balance_spot.py"
            });

            var bybitResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/bybit && source .venv/bin/activate && python balance_spot.py"
            });

            var bitgetResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
                command = "cd /var/lib/ApiGateway/cold_wallet/blockchain/cex/bitget && source .venv/bin/activate && python balance_spot.py"
            });

            var gateResponse = _httpService.PostAsync<CommonRequest, CommonResponse>("http://192.168.1.9:1704/linux/execute", new CommonRequest()
            {
