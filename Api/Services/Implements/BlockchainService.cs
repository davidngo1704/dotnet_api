...
namespace Api.Services.Implements
{
    ...
    public class BlockchainService : IBlockchainService
    {
        ...
        public async Task TriggerMinute()
        {
            ...
            var coin_list = System.Text.Json.JsonSerializer.Deserialize<List<CoinListModel>>(coin_list_string);

            var data = await _httpService.GetAsync<List<CoinListModel>>("https://api.binance.com/api/v3/ticker/price");

            foreach (var coin in coin_list!)
            {
                var price = data?.FirstOrDefault(x => x.symbol == coin.symbol)?.price;
